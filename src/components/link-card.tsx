...
function LinkCard({ url, fetchUrls }: any) {
  const downloadQR = () => {
    const imgUrl = url?.qr;
  ...
  const { user } = UrlState();
  const { loading: loadingDelete, execute: submitDelete } = useFetch(deleteUrl);
  return (
    <div className="flex flex-col md:flex-row gap-5 border p-4 bg-gray-900 rounded-lg">
      <img
        src={url?.qr}
        alt={`${url?.title} QR Code`}
      ...
      <div className="flex gap-2">
        ...
        <Button
          ...
          onClick={() => submitDelete(url?.id).then(() => fetchUrls(user?.id))}
        >
          {loadingDelete ? <BeatLoader size={6} /> : <Trash />}
        </Button>
      </div>
    </div>
  );
